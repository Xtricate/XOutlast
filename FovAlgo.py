...
###     It is code kindly shared in the
###     public domain by init, which I have 
###     minorly modified to fit my own purposes. 
###     Contact: initd5@gmail.com
...
def main(): 
    global px, py, mp, fov_mp, H, W
    # for x in range(W + 1): 
    #     fov_mp.append([])     # Reset FOV
    fov_mp = []
    ...
    for i in range(0, RAYS + 1, STEP): 
        ...
        for z in range(RAD): # Cast the ray
            ...
            if x < 0 or y < 0 or x > W or y > H: # If ray is out of range
                break
            fov_mp[int(round(x))][int(round(y))] = 1  # Make tile visible
            if mp[int(round(x))][int(round(y))].block_sight == True:  # Stop ray if it hit
                break                                     # a wall.
     
def compute(level, playerx, playery, mpheight, mpwidth):
    global mp, fov_mp, px, py, W, H
    mp = level
    px = playerx
    py = playery
    W = mpwidth
